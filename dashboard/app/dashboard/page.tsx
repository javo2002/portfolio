...
const NavItem = ({ view, icon: Icon, children, setView }) => (
    <Button variant="ghost" onClick={() => setView(view)} className="w-full justify-start gap-3 px-3 py-2 text-muted-foreground transition-all hover:text-primary hover:bg-primary/10">
      {Icon && <Icon className="h-5 w-5" />}
      <span>{children}</span>
    </Button>
)

const Sidebar = ({ setView, isAuthenticated, resumeUrl, profilePictureUrl, onSecretTrigger }) => (
    <aside className="hidden w-64 flex-col border-r border-primary/20 bg-card/80 backdrop-blur-sm sm:flex">
        <div 
            className="flex h-32 items-center justify-center border-b border-primary/20 p-4"
            onMouseDown={onSecretTrigger.start}
            onMouseUp={onSecretTrigger.end}
            onTouchStart={onSecretTrigger.start}
            onTouchEnd={onSecretTrigger.end}
            style={{ cursor: 'pointer' }}
        >
             <div className="relative w-24 h-24 glow-border">
                <img
                    src={profilePictureUrl || 'https://placehold.co/100x100/0E0F11/FAFAFA?text=ID'}
                    alt="User Profile"
        ...
        <nav className="flex flex-col gap-1 p-2 flex-1">
            <NavItem view="Dashboard" icon={Home} setView={setView}>Dashboard</NavItem>
            <NavItem view="About" icon={FileUser} setView={setView}>About</NavItem>
            <NavItem view="Education" icon={GraduationCap} setView={setView}>Education</NavItem>
            <NavItem view="Certifications" icon={Award} setView={setView}>Certifications</NavItem>
            <NavItem view="Experience" icon={Archive} setView={setView}>Experience</NavItem>
            <NavItem view="Skills" icon={ScanSearch} setView={setView}>Skills</NavItem>
            <NavItem view="Projects" icon={FileCode} setView={setView}>Projects</NavItem>
            <NavItem view="Hobbies" icon={Coffee} setView={setView}>Hobbies</NavItem>
            <NavItem view="Terminal" icon={Terminal} setView={setView}>Terminal</NavItem>
            {isAuthenticated && <NavItem view="edit" icon={Edit} setView={setView}>EDIT MODE</NavItem>}
        </nav>
        {resumeUrl && (
...
const MobileSheet = ({ setView, isAuthenticated }) => (
     <Sheet>
        <SheetTrigger asChild>
            <Button size="icon" variant="outline" className="sm:hidden"><PanelLeft className="h-5 w-5" /></Button>
        </SheetTrigger>
        <SheetContent side="left" className="sm:max-w-xs flex flex-col bg-card">
            <nav className="grid gap-6 text-lg font-medium flex-1">
                <div className="group flex h-10 w-10 shrink-0 items-center justify-center gap-2 rounded-none bg-primary text-lg font-semibold text-primary-foreground md:text-base">
                    <Laptop className="h-5 w-5 transition-all group-hover:scale-110" />
                </div>
                <NavItem view="Dashboard" icon={Home} setView={setView}>Dashboard</NavItem>
                <NavItem view="About" icon={FileUser} setView={setView}>About</NavItem>
                <NavItem view="Education" icon={GraduationCap} setView={setView}>Education</NavItem>
                ...
                {data?.content?.youtube_video_id && (
                    <Card className="animate-glitch-in" style={{animationDelay: '200ms'}}>
                        <CardHeader>
                            ...
                            <CardDescription>My Featured Content</CardDescription>
                        </CardHeader>
                        <CardContent>
        ...
        <ThemeProvider attribute="class" defaultTheme="dark" enableSystem={false}>
            <div className="flex min-h-screen w-full font-mono">
                <Sidebar 
                    ...
                    resumeUrl={portfolioData.content?.resume_url} 
                    profilePictureUrl={portfolioData.content?.profile_picture_url}
                    onSecretTrigger={{ start: handleSecretTriggerStart, end: handleSecretTriggerEnd }}
                />
                <div className="flex flex-1 flex-col">
                    <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-primary/20 bg-background/95 px-4 backdrop-blur-sm supports-[backdrop-filter]:bg-background/60">
                        <MobileSheet setView={setView} isAuthenticated={!!session} />
                    ...
                    <main className="flex-1 flex p-4 sm:p-6 gap-6">
